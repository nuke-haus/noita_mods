...
function spawn_potions(x, y) -- we just replace the default function with this one
	...
	if ( rnd >= 92 ) then -- 9 percent chance
		...
		elseif (rnd2 <= 4) then
			...
			EntityLoad( "files/nuke/entities/stone_ice.xml", x, y-2 )

		elseif (rnd2 <= 6) then

			EntityLoad( "files/nuke/entities/stone_fire.xml", x, y-2 )

		else

			EntityLoad( "files/nuke/entities/stone_holy.xml", x, y-2 )
