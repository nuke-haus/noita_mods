...
function damage_received(damage, message, entity_thats_responsible, is_fatal)
	...
	if (hp < 40 and EntityHasTag(me, "pissed_off") == false) then -- wake them up
		...
		local eff = EntityLoad("mods/nukes_altars/files/nuke/entities/gorgon_effect.xml", x, y)

		EntityAddChild(me, eff)

		local tbl = {
			script_source_file="mods/nukes_altars/files/nuke/scripts/gorgon_spawn_enemy.lua",
			execute_on_added="1",
...
if (bod ~= nil and bod ~= 0) then
	...
	if (pix < 1000) then -- they cheesed it
		...
		local baddy = EntityLoad("mods/nukes_altars/files/nuke/entities/stone_physics.xml", x, y - 4)
